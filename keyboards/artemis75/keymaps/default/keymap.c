...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
     ...
     * ├────┼────┼────┼────┼────┴────┼────┴────┼────┼────┼────┼────┼────┼────┼────┤
     * │CTRL│LALT│ GUI│ DEL│  LSHIFT │ SPACE/FN│SYMB│RALT│EMOJ│CAPS│LEFT│DOWN│RGHT│
     * └────┴────┴────┴────┴─────────┴─────────┴────┴────┴────┴────┴────┴────┴────┘
     */
    ...
    [_Q_FN] = LAYOUT(  /* QWERTY function layer */
        ...
        KC_CALC, X(BANG),  X(DEG),   _______,   X(EURO),   _______,  X(CDOT), X(BULL),  X(TIMES), _______,   X(EMPTY),    X(ENDASH), X(PLUSMIN), X(LANG),       X(RANG),
        ...
        _______, _______,  _______,  _______,   _______,             KC_CAPS,           _______,  _______,   _______,     X(NEQ),    X(LEFT),    X(DOWN),       X(RIGHT)
    ),
    /*
     * ┌────┬────┬────┬────┬────┬────┬────┬────┬────┬────┬────┬────┬────┬────┬────┐
     * │ SLP│    │    │    │    │    │    │    │    │    │    │    │    │QWTY│RSET│
     ...
     * ├────┼────┼────┼────┼────┼────┼────┼────┼────┼────┼────┼────┼────┬────┼────┤
     * │    │  { │  } │TABL│TABR│    │    │HOME│PGDN│PGUP│ END│    │    │    │MUTE│
     * ├────┼────┼────┼────┼────┴────┼────┴────┼────┼────┼────┼────┼────┼────┼────┤
     * │    │    │    │    │  SPACE  │!!!!!!!!!│    │WBAK│WFWD│    │MPRV│PLAY│MNXT│
     * └────┴────┴────┴────┴─────────┴─────────┴────┴────┴────┴────┴────┴────┴────┘
     */
    [_C_FN] = LAYOUT(  /* Colemak function/navigation layer, including keys useful for programming */
        ...
        _______, S(KC_LBRC), S(KC_RBRC), C(KC_PGUP), C(KC_PGDN), _______, _______, KC_HOME, KC_PGDN, KC_PGUP, KC_END,   _______, _______, _______,     KC_MUTE,
        _______, _______,    _______,    _______,    MT(MOD_LSFT, KC_SPC),         _______, _______, KC_WBAK, KC_WFWD,  _______, KC_MPRV, KC_MPLY,     KC_MNXT
    ),
    ...
    [_C_SYMB] = LAYOUT(  /* Colemak symbol layer */
        ...
        KC_WAKE, _______,  _______,    _______,   KC_CAPS,            _______,            _______,  KC_MS_WH_DOWN,  KC_MS_WH_UP, _______,     X(LEFT),    X(DOWN),     X(RIGHT)
    ),
...
// Shift functions as caps lock when tapped
// Based on: https://github.com/qmk/qmk_firmware/issues/3366
void dance_fc_finished(qk_tap_dance_state_t *state, void *user_data) {
    if (state->count == 1) {
        // Act as regular shift key
        register_code (KC_LSHIFT);
    } else if (state->count == 2) {
        // Double tap: Press/release caps lock
        register_code(KC_CAPS);
    }
}
void dance_fc_reset(qk_tap_dance_state_t *state, void *user_data) {
    if (state->count == 1) {
        // For a single press: when key sequence is finished, release shift
        unregister_code (KC_LSHIFT);
    } else if (state->count == 2) {
        // Double press: unregister caps lock press when finished
